<?php
	...
	function wp_sms_subscribe_meta_box() {
		add_meta_box('subscribe-meta-box', __('Subscribe SMS', 'tikab-sms'), 'wp_sms_subscribe_post', 'post', 'normal', 'high');
	}
	...
	function wp_sms_subscribe_send($post_ID) {
		...
		if(get_post_meta($post_ID, "subscribe_post", true) == 'yes') {
			...
			$obj->to = $wpdb->get_col("SELECT mobile FROM {$table_prefix}ts_subscribes");
