<?php
	...
	if(get_option('wp_notification_new_wp_version')) {
		...
		if($update[1]->current > $wp_version) {
			...
			if(get_option('wp_last_send_notification') == false) {
				...
				$obj->msg = sprintf(__('WordPress %s is available! Please update now', 'tikab-sms'), $update[1]->current);
	...
	function wps_setup_wpcf7_form($form) {
		...
		include_once dirname( __FILE__ ) . "/../templates/tikab-sms-wpcf7-form.php";
	}
	...
	function wps_send_wpcf7_sms($form) {
		...
		if( $options['message'] && $options['phone'] ) {
			...
			if( $obj->send_sms() ) {
				...
				$wpdb->query("INSERT INTO {$table_prefix}ts_send (date, sender, message, recipient) VALUES ('{$date}', '{$obj->from}', '{$obj->msg}', '{$options['phone']}')");
			}
		}
	}
